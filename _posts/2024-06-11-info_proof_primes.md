...
# Proof
Let $n$ be an integer and define $N$ as the uniform random variable whose support is the set $\left{ 1,2, \dots, n \right}$. The number of primes less than or equal to $n$ is denoted by $\pi(n)$, known as the prime-counting function. Let $p_1 < p_2 < \cdots < p_{\pi(n)}$ be the sequence of primes less than or equal to $n$. For $i \leq \pi(n)$, define a random variable $X_i$ as the exponent of $p_i$ in the prime factorization of $N$.
